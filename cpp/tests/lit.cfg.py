...
config.name = "Inline Dialect Tests"
inline_plugin = plugin('INLINE_DIALECT_PLUGIN', os.path.join(os.path.dirname(__file__), '..', 'build', 'libinline_dialect.so'))
config.substitutions.append(('opt', f'{tool('mlir-opt')} --load-dialect-plugin={inline_plugin}'))
